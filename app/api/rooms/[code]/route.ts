...
import { act, appendActionLog, applyTurnTimeout, leaveRoom, needsRoomMaintenance, publicState, startHand, upgradeRoomState } from '@/lib/poker';
...
export async function GET(request: Request, context: Context) {
  try {
    ...
    const now = Date.now();
    if (needsRoomMaintenance(room.state, now)) {
    ...
    await touchPresence(code, player.id, now);
...
export async function POST(request: Request, context: Context) {
  try {
    ...
    const presence = await readPresence(code);
    return NextResponse.json({ room: publicState(changed.state, changed.version, token, presence, now) });
  } catch (error) {
